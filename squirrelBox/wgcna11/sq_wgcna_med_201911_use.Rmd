...
data <- dataref %>% dplyr::select(c(2, 7:36)) %>%
  distinct(unique_gene_symbol, .keep_all = TRUE) %>%
  column_to_rownames("unique_gene_symbol") %>%
  as.matrix()
...
saveRDS(net, "201911_med_net.rds") #load as needed
```

```{r wgcna2}
net <- readRDS("201911_med_net.rds")
...
l <- nrow(dictn)
write.csv(dict, "201911_med_modules.csv")
MEs <- net$MEs
...
r <- round(l/5)
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_med_WGCNA_modules.tiff", width = 25, height = 4*r, dpi = 300, scale = 0.5)
```
...
library(anRichment)
modules <- read.csv("201911_med_modules.csv")
...
write.csv(GO_m, "201911_med_GOm.csv")
write.csv(KEGG_h, "201911_med_KEGGh.csv")
```
...
for(n in c(1:ncol(MEs))){
  ...
  if (length(text_temp) > 10) {
    text_temp <- text_temp[1:10]
  }
...
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_med_WGCNA_modules_KEGG.tiff", width = 25, height = 7*r, dpi = 300, scale = 0.5)
```
...
saveRDS(a, "201911med_conn_list")
write.csv(a4, "201911med_conn.csv")
...
write.csv(tophub, "201911_med_modules_hubgenes.csv")
...
write.csv(kme2, "201911_med_modules.csv")
...
write.csv(kme3, "201911_med_modules_hubgenes_corgenes.csv")
...
write.csv(kme4, "201911_med_modules_corgenestop20.csv")
```
