...
data <- read_tsv("/Users/rf/sandy/newtab/DESeq2_salmon_rlog_20191113/Hypothalamus.tsv.gz") %>%
  distinct(unique_gene_symbol, .keep_all = TRUE) %>% 
  dplyr::select(c(2, 7:36)) %>% column_to_rownames("unique_gene_symbol") %>% as.matrix()
data <- data[,order(colnames(data))]

cond <- read.csv("/Users/rf/sandy/hypothalamus_phenodata.csv",row.names="ids")
...
namevec <- str_c("H", condfull$Animal)
...
saveRDS(net, "201911_hy_net.rds") #load as needed
```

```{r wgcna2}
net <- readRDS("201911_hy_net.rds")
...
l <- nrow(dictn)
write.csv(dict, "201911_hy_modules.csv")
MEs <- net$MEs
...
r <- round(l/5)
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_hy_WGCNA_modules.tiff", width = 25, height = 4*r, dpi = 300, scale = 0.5)
```
...
library(anRichment)
modules <- read.csv("201911_hy_modules.csv")
...
write.csv(GO_m, "201911_hy_GOm.csv")
write.csv(KEGG_h, "201911_hy_KEGGh.csv")
```
...
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_hy_WGCNA_modules_KEGG.tiff", width = 25, height = 7*r, dpi = 300, scale = 0.5)
```
...
saveRDS(a, "201911hy_conn_list")
write.csv(a4, "201911hy_conn.csv")
...
write.csv(tophub, "201911_hy_modules_hubgenes.csv")
...
write.csv(kme2, "201911_hy_modules.csv")
...
write.csv(kme3, "201911_hy_modules_hubgenes_corgenes.csv")
...
write.csv(kme4, "201911_hy_modules_corgenestop20.csv")
