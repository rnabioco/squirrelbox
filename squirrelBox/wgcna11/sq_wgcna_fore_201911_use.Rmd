...
dataref <- read_tsv("/Users/rf/sandy/newtab/DESeq2_salmon_rlog_20191113/Forebrain.tsv.gz") %>%
  distinct(unique_gene_symbol, .keep_all = TRUE)
data <- read_tsv("/Users/rf/sandy/newtab/DESeq2_salmon_rlog_20191113/Forebrain.tsv.gz") %>%
  distinct(unique_gene_symbol, .keep_all = TRUE) %>% 
  dplyr::select(c(2, 7:36)) %>% column_to_rownames("unique_gene_symbol") %>%
  as.matrix()
data <- data[,order(colnames(data))]

cond <- read.csv("/Users/rf/sandy/brain_phenodata.csv",row.names="ids")
...
namevec <- str_c("B", condfull$Animal)
...
# td <- td > 7
# td <- td %>% as.data.frame()
# td$group <- condfull[["Group"]]
# td2 <- td %>% group_by(group) %>% summarize_all(sum)
# td2 <- td2 %>% column_to_rownames("group")
# td3 <- lapply(td2, max)
# td4 <- names(td3[td3 >= 4])
# 
# # gene_list <- rownames(data)[(rowSums(data) / ncol(data)) >= 6]
# data_fil25 <- data[rownames(data) %in% td4,] 
# dim(data_fil25) #12855 left

#remove variation from sex
batch <- cond$sex
modcombat <- model.matrix(~1, data = cond)
combat <- ComBat(dat = data, batch = batch, mod = modcombat, par.prior=TRUE, prior.plots=FALSE)
...
saveRDS(net, "201911_fore_net.rds") #load as needed
```

```{r wgcna2}
net <- readRDS("201911_fore_net.rds")
...
l <- nrow(dictn)
write.csv(dict, "201911_fore_modules.csv")
MEs <- net$MEs
...
saveRDS(gg, "fore_gg11")
#cowplot::plot_grid(plotlist = gg, ncol = 5)
r <- round(l/5)
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_fore_WGCNA_modules.tiff", width = 25, height = 4*r, dpi = 300, scale = 0.5)
```
...
options(stringsAsFactors = FALSE)
library(anRichment)
modules <- read.csv("201911_fore_modules.csv")
...
write.csv(GO_m, "201911_fore_GOm.csv")
write.csv(KEGG_h, "201911_fore_KEGGh.csv")
```
...
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201911_fore_WGCNA_modules_KEGG.tiff", width = 25, height = 7*r, dpi = 300, scale = 0.5)
```
...
saveRDS(a, "201911fore_conn_list")
write.csv(a4, "201911fore_conn.csv")
...
write.csv(tophub, "201911_fore_modules_hubgenes.csv")
...
write.csv(kme2, "201911_fore_modules.csv")
...
write.csv(kme3, "201911_fore_modules_hubgenes_corgenes.csv")
...
write.csv(kme4, "201911_fore_modules_corgenestop20.csv")
