...
dataref <- read_tsv("/Users/rf/sandy/newtab/DESeq2_salmon_rlog_20190514/Forebrain.tsv.gz")
data <- read_tsv("/Users/rf/sandy/newtab/DESeq2_salmon_rlog_20190514/Forebrain.tsv.gz") %>% dplyr::select(c(2, 7:36)) %>% column_to_rownames("unique_gene_symbol") %>% as.matrix()
...
saveRDS(net, "201909_fore_net.rds") #load as needed
```

```{r wgcna2}
net <- readRDS("201909_fore_net.rds")
...
l <- nrow(dictn)
write.csv(dict, "201909_fore_modules.csv")
MEs <- net$MEs
...
saveRDS(gg, "fore_gg09")
#cowplot::plot_grid(plotlist = gg, ncol = 5)
r <- round(l/5)
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201909_fore_WGCNA_modules.tiff", width = 25, height = 4*r, dpi = 300, scale = 0.5)
```
...
options(stringsAsFactors = FALSE)
library(anRichment)
modules <- read.csv("201909_fore_modules.csv")
...
write.csv(GO_m, "201909_fore_GOm.csv")
write.csv(KEGG_h, "201909_fore_KEGGh.csv")
```
...
ggsave(cowplot::plot_grid(plotlist = gg, ncol = 5), file = "201909_fore_WGCNA_modules_KEGG.tiff", width = 25, height = 7*r, dpi = 300, scale = 0.5)
```
...
saveRDS(a, "201909fore_conn_list")
write.csv(a4, "201909fore_conn.csv")
...
write.csv(tophub, "201909_fore_modules_hubgenes.csv")
...
write.csv(kme2, "201909_fore_modules.csv")
...
write.csv(kme3, "201909_fore_modules_hubgenes_corgenes.csv")
...
write.csv(kme4, "201909_fore_modules_corgenestop20.csv")
