...
#' d <- tibble(
#'   value = c(1, 2, 3, 5, 6, 7, 8, 9, 10, 12, 13),
...
gen_circle_2 <- function() {
  rbind(
    gen_circle(1L, -2 / 3, 0, 1),
    gen_circle(2L, 2 / 3, 0, 1)
  )
}
gen_text_pos_2 <- function() {
  tribble(
    ~name, ~x, ~y, ~hjust, ~vjust, ~A, ~B,
...
prepare_venn_data <- function(data, columns = NULL,
  ...
  if (is.data.frame(data)) {
    ...
    if (length(columns) == 2) {
      ...
      stopifnot((d1 %>% select(-n) %>% count(A, B, name = "nn") %>% with(nn)) == 1)
      for (i in 1:nrow(d1)) {
      ...
      stopifnot((d1 %>% select(-n) %>% count(A, B, C, name = "nn") %>% with(nn)) == 1)
      for (i in 1:nrow(d1)) {
        idx <- ((!xor(d1$A[[i]], as_tibble(data)[, columns[[1]]])) &
          (!xor(d1$B[[i]], as_tibble(data)[, columns[[2]]])) &
      ...
      stopifnot((d1 %>% select(-n) %>% count(A, B, C, D, name = "nn") %>% with(nn)) == 1)
      for (i in 1:nrow(d1)) {
